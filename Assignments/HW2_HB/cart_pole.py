...
class CartPole(Environment):
    ...
    def __init__(
        ...
        high = np.array([np.pi * 0.5, 3 * np.pi])
        self.high = high
    ...
    def reset(self, state=None):
        if state is None:
            angle = np.random.uniform(-np.pi / 8.0, np.pi / 8.0)
    ...
    def step(self, action):
        ...
        if (
            np.abs(self._state[0]) > np.pi * 0.5
            or np.abs(self._state[1]) > self.high[1]
        ):
            reward = -10.0
            absorbing = True
        else:
            reward = 0.1 * (np.pi / 8 - np.abs(self._state[0])) + 0.05 * (
                np.pi / 8 - np.abs(self._state[1])
            )
            absorbing = False
    ...
    def _dynamics(self, state, t, u):
        ...
        d_omega = (
            ...
            * 0.5
            ...
            2 / 3 * self._l - self._alpha * self._m * self._l * 0.5 * np.cos(theta) ** 2
        )
